...
void NonPenetration::pre_solve(const float dt) {
  const math::Vec2 point_a = a_->local_to_world_point(point_a_local_);
  const math::Vec2 point_b = b_->local_to_world_point(point_b_local_);
  const math::Vec2 n = a_->local_to_world_vector(normal_a_local_);

  jacobian_ = math::MatMN<1, 6>{
      -n.x,
      -n.y,
      -math_utils::cross(n, point_a - a_->position),
      n.x,
      n.y,
      math_utils::cross(n, point_b - b_->position),
  };
  ...
  float c = math::dot(point_b - point_a, -n);
